...
import { Button, Card, Col, Form as AntdForm, Row, message } from "antd"
...
import { DestinationEditProps, DestinationJob } from "@app-types/index"
import {
...
import { DESTINATION_TYPES, DEFAULT_DESTINATION_TYPE, transformErrors } from "@utils/constants"
...
const DestinationEditComponent: React.FC<DestinationEditProps> = ({
  fromJobFlow = false,
  onSuccess,
  onCancel,
}) => {
  ...
  const [connectorType, setConnectorType] = useState<string>(
	...
	const handleTestConnection = async () => {
		try {
			...
			if (
				response.data.connection_result.status ===
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center space-x-4">
					<Button
						...
						onClick={() =>
							fromJobFlow && onCancel ? onCancel() : navigate(-1)
						}
					/>
					<div>
						<Typography.Title
...
const DestinationEdit = withAbortController(DestinationEditComponent)
